...
Given the circular array code and an integer key k, r
    eturn the decrypted code to defuse the bomb!
...
export function decrypt(code: number[], k: number): number[] {
    ...
    if (k > 0) {
        ...
        // Calculate the initial sum of the last `k` numbers
    ...
    // Sliding window loop
    for (let i = 1; i < n; i++) {
        let subtractIdx, addIdx;

        if (k > 0) {
            // Move the window forward for k > 0
            subtractIdx = i;
            addIdx = (i + k) % n;
        } else {
            // Move the window backward for k < 0
            subtractIdx = (n + i - absK - 1) % n;
            addIdx = (n + i - 1) % n;
        }

        sum += code[addIdx] - code[subtractIdx];
        output[i] = sum;
    }
